...
interface CenterPanelProps {
  ...
  onSelectCard: (card: TcgCard | null) => void
  // Registration
...
export default memo(function CenterPanel({
  ...
  const filteredIds = useMemo(() => filteredCards.map(c => String(c.tcgplayer_id)), [filteredCards])
  ...
  return (
    <main className={`flex flex-col overflow-hidden ${className}`}>
      ...
      <div className="p-3 border-b border-[var(--jtcg-border)] bg-[var(--jtcg-surface)] shrink-0">
        ...
        {/* 登録ツールバー */}
        {showRegistration && (filteredCards.length > 0 || bulkProgress) && (
                    ...
                    <div className="flex items-center gap-1.5">
                      {readyCount > 0 && (
                        <button
                          ...
                          className="text-xs px-3 py-1.5 rounded-[var(--jtcg-radius)] bg-[var(--jtcg-ink)] text-white font-bold hover:opacity-90"
                        >
                          {`${readyCount}件を登録`}
                        </button>
                      )}
                      {readyOverwriteCount > 0 && (
                        <button
                          ...
                          className="text-xs px-3 py-1.5 rounded-[var(--jtcg-radius)] bg-amber-500 text-white font-bold hover:opacity-90"
                        >
              ...
              return (
                ...
                >
                  <CardListItem
                    ...
                    isChecked={checkedCards.has(String(card.tcgplayer_id))}
                    isRegistered={!!registered[String(card.tcgplayer_id)]}
