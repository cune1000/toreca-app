...
export async function GET(request: NextRequest) {
  try {
    ...
    const patterns = Array.from(patternMap.entries())
      ...
      .map(([recognized, { corrected, count }]) => ({ recognized, corrected, count }))
      .sort((a, b) => b.count - a.count)
      .slice(0, limit)

    return NextResponse.json({ success: true, patterns, total: patterns.length })
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: true, deleted: recognizedName })
