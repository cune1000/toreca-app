...
async function scrapeTorecaCamp(url: string) {
  ...
  try {
    ...
    const data = await page.evaluate(() => {
      ...
      return { name, mainPrice, conditions, imageUrl }
    })
  ...
  try {
    ...
    switch (detectedSource) {
      ...
      case 'torecacamp':
        ...
        break
      default:
        return NextResponse.json({ error: 'Unknown source. Supported: snkrdunk, torecacamp' }, { status: 400 })
    }
...
export async function GET(request: NextRequest) {
  ...
  if (!url) {
    return NextResponse.json({ 
      message: 'Price scraping API (local Playwright)',
      usage: 'POST with { url: "https://..." } or GET with ?url=https://...',
      supported: ['snkrdunk.com', 'torecacamp-pokemon.com']
    })
  }
