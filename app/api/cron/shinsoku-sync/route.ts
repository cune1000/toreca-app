...
import { fetchShinsokuItems } from '@/lib/shinsoku'
    ...
    try {
        ...
        for (const brand of brands) {
            try {
                ...
                console.log(`[shinsoku-sync] ${uniqueItems.length} unique items for ${brand}`)
                const batchSize = 500
        ...
        if (logError) {
            console.error('[shinsoku-sync] cron_logs insert error:', logError.message)
        }

        await markCronJobRun('shinsoku-sync', errors.length > 0 ? 'error' : 'success', errors.length > 0 ? errors.join(', ') : undefined)
        ...
        console.error('[shinsoku-sync] Cron error:', error)
        await markCronJobRun('shinsoku-sync', 'error', message).catch(() => {})
