...
export async function POST(request: NextRequest) {
  try {
    ...
    const { tcgplayer_ids } = body
    if (!Array.isArray(tcgplayer_ids)) {
      return NextResponse.json({ success: false, error: 'tcgplayer_ids は配列で指定してください' }, { status: 400 })
    }

    // 型チェック + 重複除去 + 上限ガード
    const ids = [...new Set(
      tcgplayer_ids.filter((id): id is string => typeof id === 'string' && id.length > 0 && id.length <= 200)
    )].slice(0, MAX_IDS)
    ...
    for (let i = 0; i < ids.length; i += CHUNK_SIZE) {
      ...
      if (data) {
        for (const row of data) {
          if (row.tcgplayer_id) {
            registeredIds.push(row.tcgplayer_id)
            if (row.name) registeredNames[row.tcgplayer_id] = row.name
          }
        }
      }
    }
